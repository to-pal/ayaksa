...
abstract class modCarouselHelper
{
	public static function getList(&$params)
	{
		...
		foreach ($items as &$item)
		{
			...
			if ($show_introtext) {
				...
				$item->introtext = JHtml::_('content.prepare', $item->introtext, '', 'mod_carousel.content');
	...
	*/
	...
	public static function html_image($images, $types = array('image_intro', 'image_fulltext'), $caption = false){
		$image_opp = json_decode($images, true);
		$image = null;
		$image_html = ""; 
		foreach($types as $key=>$type){
			if( $image_opp[$type] != null ){
				$image = $image_opp[$type]; break;
			}
		}
		if($image == null){
				$image = self::getplaceholder('mod_carousel');
		}
		return $image;
	}
	/**
	 ...
	 * @return string
	 */
	public static function getplaceholder($module_name){
		return 'modules/'.$module_name.'/assets/images/nophoto.png';
	}
}

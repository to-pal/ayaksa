...
/**
 * Helper for mod_sj_extraslider
 *
 * @package     Joomla.Site
 * @subpackage  mod_sj_extraslider
 */
abstract class SjExtrasliderHelper
{
	...
	public static function getImages($item, $params, $ctype='article'){
		...
		if ( !isset(self::$image_cache[$hash][$item->id]) ){
			...
			if ( count($images_path) == 0 && $params->get('imgcfg_placeholder', 1)==1){
				$images_path[] = array('src'=> $params->get('imgcfg_placeholder_path', 'modules/mod_sj_extraslider/assets/images/nophoto.png'), 'class'=>'placeholder');
			}

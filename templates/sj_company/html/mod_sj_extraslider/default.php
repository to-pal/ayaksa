 ...
 */
    ...
    JHtml::stylesheet('templates/'.$template.'/html/mod_sj_extraslider/css/style.css');
    JHtml::stylesheet('templates/'.$template.'/html/mod_sj_extraslider/css/css3.css');
    ...
    <div id="<?php echo $tag_id;?>" class="sj-extraslider <?php if( $options->effect == 'slide' ){ echo $options->effect;}?> preset02-<?php echo $options->num_cols; ?>" data-start-jcarousel='1'>
    	...
    	<div class="extraslider-control  <?php echo $options->theme; ?> <?php if( $options->button_page == 'under' ){echo 'button-type2';}?>">
	 ...
	 <?php if($options->title_slider_display == 1){?>
	    <div class="extraslider-heading  <?php echo $options->theme; ?>">
	...
	<div class="extraslider-inner <?php echo $options->theme; ?>">
                ...
                <?php }?>  
				    ...
				    <div class="item-wrap <?php echo $options->theme; if($count%$options->num_cols == 0 || $count== $count_item && $options->num_cols !=1){echo " last";}?> ">
				    	<div class="item-image">
							<a class="info" href="<?php echo $item->link;?>" title="<?php echo $item->title;?>" target = "<?php echo $options->item_link_target;?>">
	    ...
	    <ul class="nav-page nav-under  <?php echo $options->theme; ?>">
	    <?php $j = 0;$page = 0;
	    	foreach ($items as $item){$j ++;
			$active_class = $page == 0 ? " active" : "";
	    		if( $j%$item_of_page == 1 || $item_of_page == 1 ){$page ++;?>

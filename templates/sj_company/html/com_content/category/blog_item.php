...
?>
	...
	$imgsrc = YTTemplateUtils::resize($images->image_intro, $imgH, $imgW, array('function'=>'fill' ,'background' => $templateParams->get('thumbnail_background', '#ffffff')));
	...
	?>
	<div class="img-intro-<?php echo htmlspecialchars($imgfloat); ?> item-image"> 
		<img <?php if ($images->image_intro_caption): echo 'class="caption"'.' title="' .htmlspecialchars($images->image_intro_caption) .'"'; endif; ?>	src="<?php echo htmlspecialchars($imgsrc); ?>"<?php echo $imgattr; ?> alt="<?php echo htmlspecialchars($images->image_intro_alt); ?>" /> 
	</div>
	<?php endif; 
	// End: dungnv edited
	?>
			...
			<?php if ($useDefList AND ($info == 1 OR $info == 2)) : ?>
				<div class="article-info muted">
					...
					<?php if ($info == 1): ?>
						...
						<?php 
							//Bo doan nay
							if ($params->get('show_category')==10) : ?>
							<dd>
								<div class="category-name">
									<?php 	$title = $this->escape($this->item->category_title);
									$url = '<a href="'.JRoute::_(ContentHelperRoute::getCategoryRoute($this->item->catslug)).'">'.$title.'</a>';?>

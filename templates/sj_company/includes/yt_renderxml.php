...
Class YtRenderXML {		
			...
			foreach($block->childNodes as $positions ): if($positions->nodeName!='#text'){
				...
				foreach($positions->childNodes as $position):
					if($position->nodeName!='#text'){
						...
						// No left
						if($position->hasAttribute('noleft-class')){
							$posit['noleft-class'] = $position->getAttribute('noleft-class');
						}
						if($position->hasAttribute('noleft-data-wide')){
							$posit['noleft-data-wide'] = $position->getAttribute('noleft-data-wide');
						}
						if($position->hasAttribute('noleft-data-normal')){
							$posit['noleft-data-normal'] = $position->getAttribute('noleft-data-normal');
						}
						if($position->hasAttribute('noleft-data-tablet')){
							$posit['noleft-data-tablet'] = $position->getAttribute('noleft-data-tablet');
						}
						if($position->hasAttribute('noleft-data-stablet')){
							$posit['noleft-data-stablet'] = $position->getAttribute('noleft-data-stablet');
						}
						if($position->hasAttribute('noleft-data-mobile')){
							$posit['noleft-data-mobile'] = $position->getAttribute('noleft-data-mobile');
						}
						// No right
						if($position->hasAttribute('noright-class')){
							$posit['noright-class'] = $position->getAttribute('noright-class');
						}
						if($position->hasAttribute('noright-data-wide')){
							$posit['noright-data-wide'] = $position->getAttribute('noright-data-wide');
						}
						if($position->hasAttribute('noright-data-normal')){
							$posit['noright-data-normal'] = $position->getAttribute('noright-data-normal');
						}
						if($position->hasAttribute('noright-data-tablet')){
							$posit['noright-data-tablet'] = $position->getAttribute('noright-data-tablet');
						}
						if($position->hasAttribute('noright-data-stablet')){
							$posit['noright-data-stablet'] = $position->getAttribute('noright-data-stablet');
						}
						if($position->hasAttribute('noright-data-mobile')){
							$posit['noright-data-mobile'] = $position->getAttribute('noright-data-mobile');
						}
						// No leftright
						if($position->hasAttribute('noleftright-class')){
							$posit['noleftright-class'] = $position->getAttribute('noleftright-class');
						}
						if($position->hasAttribute('noleftright-data-wide')){
							$posit['noleftright-data-wide'] = $position->getAttribute('noleftright-data-wide');
						}
						if($position->hasAttribute('noleftright-data-normal')){
							$posit['noleftright-data-normal'] = $position->getAttribute('noleftright-data-normal');
						}
						if($position->hasAttribute('noleftright-data-tablet')){
							$posit['noleftright-data-tablet'] = $position->getAttribute('noleftright-data-tablet');
						}
						if($position->hasAttribute('noleftright-data-stablet')){
							$posit['noleftright-data-stablet'] = $position->getAttribute('noleftright-data-stablet');
						}
						if($position->hasAttribute('noleftright-data-mobile')){
							$posit['noleftright-data-mobile'] = $position->getAttribute('noleftright-data-mobile');
						}
	...
	function updateTagBodyContent($tagBody){
		...
		$tagBody['class_content'] = 'content layout-'.$this->layouttype;
